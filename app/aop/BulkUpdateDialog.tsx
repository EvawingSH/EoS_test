...
export interface BulkUpdateData {
  ...
  criticalServiceRemediationPlan: string // New field for critical services
...
const BulkUpdateDialog: React.FC<BulkUpdateDialogProps> = ({ open, onOpenChange, selectedItems, onBulkUpdate }) => {
  ...
  // State to track if any critical services are selected
  const [hasCriticalServices, setHasCriticalServices] = useState(false)
  ...
  // Check if any selected services are critical
  useEffect(() => {
    if (selectedItems.length > 0) {
      const criticalServices = servicesData.services.filter(
        (service) => selectedItems.includes(service.id) && service.category === "Critical",
      )
      setHasCriticalServices(criticalServices.length > 0)
    } else {
      setHasCriticalServices(false)
    }
  }, [selectedItems])
  ...
  const resetBulkUpdateForm = () => {
    setBulkUpdateData({
      ...
      eosConditionsForNonOnboardedAssets: "",
      impactIfCompromised: "",
      criticalServiceRemediationPlan: "",
          ...
          {hasCriticalServices && (
            <div className="bg-amber-50 border-l-4 border-amber-500 p-4 mb-6 rounded-r-md">
              <div className="flex items-start">
                ...
                <div>
                  <p className="text-amber-800 font-medium">Your selection includes critical services</p>
                  ...
                  {hasCriticalServices && (
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <Textarea
                        ...
                        value={bulkUpdateData.criticalServiceRemediationPlan}
                        onChange={handleInputChange}
                        placeholder="Describe the detailed remediation plan for critical services"
