...
export default function EditItemForm({ id, initialService }: EditItemFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    impactIfCompromised: "",
    criticalServiceRemediationPlan: "", // New field for critical services
  ...
  useEffect(() => {
    if (initialService) {
      ...
      const plannedDate = formatDateForDisplay(initialService.eosDate)
      const reviewDateFormatted = formatDateForDisplay("2025-10-15") // Default date

      // Set the form data with the service information
      setFormData({
        // General Information
        serviceCI: initialService.id,
        serviceName: initialService.name,
        serviceManager: initialService.serviceManager,
        serviceOwner: initialService.serviceOwner,
        division: initialService.division,
        sensitivityTier: initialService.sensitivityTier,
        category: initialService.category,
        businessRisk: initialService.risk,

        // End of Support
        rasScore: initialService.rasScore,
        residualScore: initialService.residualScore,

        // EoS Remediation Plan
        eosDate: initialService.eosDate,
        plannedAddressDate: plannedDate, // Format as DD/MM/YYYY
        riskFY: initialService.plan, // Use the plan value from the service data
        ...
        eosConditionsForNonOnboardedAssets: "",
        impactIfCompromised: "",
        criticalServiceRemediationPlan: "", // Initialize empty

        // Tech & Cyber - using default values since these are new fields
        techCyberReviewStatus: initialService.techCyberReviewStatus,
        ...
        // Other fields
        incompletePlan: initialService.incompletePlan,
        remediationExpired: initialService.remediationExpired,
        invalidSelection: initialService.invalidSelection,
      })
    }
  }, [initialService])
  ...
  const isCritical = service?.category === "Critical"
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="mb-4">The service with ID {id} could not be found.</p>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          {remediationPlanExpanded && (
            <div className="p-6 transition-all duration-300">
              ...
              {isCritical && (
                <div className="bg-amber-50 border-l-4 border-amber-500 p-4 mb-6 rounded-r-md">
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-amber-800 font-medium">Your service includes critical software</p>
                      <p className="text-amber-700 text-sm mt-1">
                        Critical services require special attention and detailed remediation plans.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* EoS Date Box for Critical Services */}
              {isCritical && (
                <div className="bg-gray-100 p-4 mb-6 rounded-md border border-gray-300">
                  <h4 className="font-medium text-gray-800 mb-2">End of Support Date</h4>
                  <p className="text-lg font-bold text-red-600">{formatDateForDisplay(service.eosDate)}</p>
                  <p className="text-sm text-gray-600 mt-1">
                    This critical service will reach end of support on the date above.
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                {/* Critical Service Remediation Plan */}
                {isCritical && (
                  <div className="space-y-1 col-span-3">
                    <Label htmlFor="criticalServiceRemediationPlan" className="text-sm font-medium text-red-600">
                      Please provide the remediation plan regarding the critical services
                    </Label>
                    <Textarea
                      id="criticalServiceRemediationPlan"
                      name="criticalServiceRemediationPlan"
                      value={formData.criticalServiceRemediationPlan}
                      onChange={handleInputChange}
                      placeholder="Describe the detailed remediation plan for this critical service"
                      className="min-h-[120px] border-amber-300 focus-visible:ring-amber-500"
                    />
                  </div>
                )}
